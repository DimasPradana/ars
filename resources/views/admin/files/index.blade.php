...
@section('content')
	...
	@can('file_create')
		...
		@if (Auth::getUser()->role_id == 2 && $userFilesCount >= 50000)
				...
				<thead>
					<tr>
						@can('file_delete')
							...
							{{-- TODO dimas Rab 10 Jul 2019 11:00:46  WIB --}}
							{{-- - tambah kode barang --}}
							{{-- - ubah nama folder jadi rak --}}
							<th>Filename</th>
							<th>Uraian</th>
							<th>Kode Arsip</th>
							<th>Nomor Arsip</th>
							...
							<tr data-entry-id="{{ $file->id }}">
								@can('file_delete')
									...
									<td field-key='filename'> @foreach($file->getMedia('filename') as $media)
										<p class="form-group">
										{{-- TODO dimas Sel 16 Jul 2019 03:27:06  WIB --}}
										{{--     - jika nama file di klik open file without download --}}
										{{-- <a href="{{url('/admin/' . $file->uuid . '/download')}}" target="_blank">{{ $media->name }} ({{ $media->size }} KB)</a> --}}
										<a href="{{url('/admin/' . $file->uuid . '/show')}}" target="_blank">{{ $media->name }} ({{ $media->size }} KB)</a>
										</p>
									@endforeach
									</td>
									<td>{{ $file->caption }}</td>
									<td field-key='created_by'>
										{{-- dimas --}}
										{{ $file->created_by->name }}/{{ $file->folder->name }}/{{ $file->id }}
									</td>
									<td>{{ $file->archiveNumbers }}</td>
